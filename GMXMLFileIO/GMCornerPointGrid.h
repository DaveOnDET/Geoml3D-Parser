...
class vtkDataArray;
class GMXMLFILEIO_EXPORT GMCornerPointGrid : public vtkUnstructuredGrid
{
public:
	...
	vtkTypeMacro(GMCornerPointGrid, vtkUnstructuredGrid);
...
private:
	//创建K方向第N层角点网格
	void MakeKthGrid(int k, vtkUnstructuredGrid*out);
	//计算指定Pillar给定深度值下的角点坐标
	inline void calcCornerPoint(int pillarIndex, double depth, double pos[3]);
protected:
	std::string         _ID;
	int                 Dimension[3];//Ni*Nj*Nk;
	std::vector<Pillar>       Pillars; //Pillars.size() = (Ni+1)*(Nj+1);
